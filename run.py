...
def api(raw_image: list, status: str, head_angles: tuple, best_time):
    ...
    if status == 'good':
        pos = "in a good position"
    elif status == 'bad':
        pos = "in a bad position"
    ...
    prompt = f"""You are PosturePal, a friendly and humourous assistant that detects the user's 
        ...
        currectly sitting {pos} (this might be wrong, refer to the image for accuracy) and the best the 
...
def main():
    ...
    resnet = posenet.get_resnet(height) # no residual network is used, geom. model used instead
    ...
    while True:
        ...
        for pi, (pose_score, keypoint_score, keypoint_coord) in enumerate(zip(pose_scores, keypoint_scores, keypoint_coords)):
            if pose_score > 0.15:
                ...
                if len(recent_rolls) > window_size:
                    recent_rolls = recent_rolls[-window_size:]
                if len(recent_pitches) > window_size:
                    recent_pitches = recent_pitches[-window_size:]

        if frame_count % 10 == 0 and frame_count != 0 and len(recent_rolls) > 0:
            average_roll = sum(recent_rolls) / len(recent_rolls)
            average_pitch = sum(recent_pitches) / len(recent_pitches)
            
            print(f"Average Roll (last {len(recent_rolls)}): {average_roll:.2f} degrees, Average Pitch (last {len(recent_pitches)}): {average_pitch:.2f} degrees")
            
            if average_roll <= 10 and average_roll >= -10 and average_pitch <= 10 and average_pitch >= -10:
        ...
        if len(recent_rolls) > 0 and len(recent_pitches) > 0:
            avg_roll_display = sum(recent_rolls) / len(recent_rolls)
            avg_pitch_display = sum(recent_pitches) / len(recent_pitches)
            
            cv2.putText(overlay_image, f"Roll: {avg_roll_display:.1f} deg", (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(overlay_image, f"Pitch: {avg_pitch_display:.1f} deg", (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            if avg_roll_display <= 10 and avg_roll_display >= -10 and avg_pitch_display <= 10 and avg_pitch_display >= -10:
                status_text = "GOOD POSTURE"
                color = (0, 255, 0)
            else:
                status_text = "ADJUST POSTURE"
                color = (0, 0, 255)
            cv2.putText(overlay_image, status_text, (10, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
            
            cv2.putText(overlay_image, f"Best: {best_time:.0f}s", (10, 120), 
    ...
    if len(roll_history) > 0:
        final_avg_roll = sum(roll_history) / len(roll_history)
        final_avg_pitch = sum(pitch_history) / len(pitch_history)
        print(f"Final Average Roll (all): {final_avg_roll:.2f} degrees")
        print(f"Final Average Pitch (all): {final_avg_pitch:.2f} degrees")
